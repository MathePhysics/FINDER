...
%Set MA to minimum truncation value which optimal WrongPoints value attains
%Find all corresponding Mres which optimal WrongPoints value attains
[parameters, iMres] = plotHeatMap(WrongPoints, parameters);

%Extract Separation Criteria corresponding to Best MA and corresponding
%iMres
SepCrit2 = SepCrit(parameters.snapshots.k1, iMres);

%Set Mres corresponding to best Separation Criterion
parameters = plotHeatMap3(SepCrit2, parameters, iMres);
...
SCB = eye(size(D.T.CovA)) .* sqrt(Sigma);
SCB = SCB.^2;
...
%==========================================================================
function [parameters, iMres] = plotHeatMap(Data, parameters)
...
%==========================================================================
function parameters = plotHeatMap3(SepCrit2, parameters, iMres)


figure('Name', 'Separation Criterion')

x = find(iMres);
plot(find(iMres), SepCrit2, 'LineWidth', 2)
[Record, imin] = min(SepCrit2);

title(sprintf('Mres = %d, SC = %0.3e', imin, Record))
set(gca, 'YScale', 'log')

parameters.multilevel.BestMres = x(imin);
Mres = [parameters.multilevel.Mres, parameters.multilevel.BestMres];
Mres = unique(Mres);
