function [data2] = Space_Slice_Mean(data_EVI,parameters)
Sx=500;
parpool(28);
ti=tic;
...
sm=zeros(1,T);
for u=1:T
    non_nan=sum(~isnan(data_EVI(:,:,u)),"all");
    data_holder=data_EVI(:,:,u);
    data_holder(isnan(data_holder))=0;
    sm(u)=sum(data_holder,"all")/max(non_nan,1);
end
for q=0:floor(L/Sx)
q
L1=max(Sx*q-s,1);
